...
unsigned tree_sitter_haxe_external_scanner_serialize(bool *just_saw_brace,
                                                     char *buffer) {
  // printf("serialising\n");
...
void tree_sitter_haxe_external_scanner_deserialize(bool *just_saw_brace,
                                                   const char *buffer,
                                                   unsigned length) {
  if (length > 0) {
...
bool tree_sitter_haxe_external_scanner_scan(bool *just_saw_brace,
                                            TSLexer *lexer,
  ...
  if (valid_symbols[CLOSING_BRACE_UNMARKER] && *just_saw_brace &&
      !(lexer->lookahead == '}' || is_whitespace(lexer->lookahead))) {
    *just_saw_brace = false;
